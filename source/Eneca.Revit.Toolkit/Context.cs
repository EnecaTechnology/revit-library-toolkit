...
public static class Context
{
	...
	static Context()
	{
		...
		var apiAssembly = AppDomain.CurrentDomain.GetAssemblies()
			.First(assembly => assembly.GetName().Name == "APIUIAPI");
		var dbAssembly = AppDomain.CurrentDomain.GetAssemblies()
			.First(assembly => assembly.GetName().Name == "RevitDBAPI");
		...
		var getApplicationMethod =
			dbAssemblyMethods.FirstOrDefault(info => info.Name == "RevitApplication.getApplication_");
		ThrowIfApiModified(getApplicationMethod is null);

		var proxyType = dbAssembly.DefinedTypes.FirstOrDefault(info =>
			info.FullName == "Autodesk.Revit.Proxy.ApplicationServices.ApplicationProxy");
		...
		var apiCallDepthManagerMethod =
			apiAssemblyMethods.FirstOrDefault(info => info.Name == "APICallDepthManager.singletonfactory");
		ThrowIfApiModified(apiCallDepthManagerMethod is null);

		var isRevitInApiModeMethod =
			apiAssemblyMethods.FirstOrDefault(info => info.Name == "APICallDepthManager.isRevitInAPIMode");
	...
	/// <summary>
	///     Represents the database level Autodesk Revit Application, providing access to documents, options and other
	...
	/// <summary>Represents a currently active Autodesk Revit project at the UI level.</summary>
	[Obsolete(
		"Document property renamed and will be removed in the next Major version, use Context.ActiveUiDocument instead")]
	...
	///     The active or top most view will be the active project and hence the active document which is available from the
	...
	/// <summary>Represents a currently active Autodesk Revit project at the database level.</summary>
	[Obsolete(
		"Document property renamed and will be removed in the next Major version, use Context.ActiveDocument instead")]
	...
	///         <ul>
	///             <li>There is no open transaction.</li>
	///             <li><see cref="P:Autodesk.Revit.DB.Document.IsModifiable" /> is false.</li>
	///             <li><see cref="P:Autodesk.Revit.DB.Document.IsReadOnly" /> is false.</li>
	///             <li>
	///                 ViewActivating, ViewActivated, and any pre-action of events (such as DocumentSaving or DocumentClosing
	...
	///         <li>If the given view is a template view; -or-</li><li>If the given view is an internal view.</li>
	///     </ul>
	/// </exception>
	...
	///         When setting the property:
	///         <ul>
	///             <li>If the document is not currently active; -or-</li>
	///             <li>If the document is currently modifiable (i.e. with an active transaction); -or-</li>
	///             <li>If the document is currently in read-only state; -or-</li>
	///             <li>When invoked during either ViewActivating or ViewActivated event; -or-</li>
		...
		set
		{
			if (UiApplication.ActiveUIDocument is null)
				throw new InvalidOperationException(
					"There are no active documents in the current Autodesk Revit session");
	...
	///     Set <see langword="true" /> if errors should be automatically resolved, otherwise <see langword="false" /> to
	...
	private static void ResolveFailures(object? sender, FailuresProcessingEventArgs args)
	{
		...
		var result = _suppressFailureErrors
...
#endif
	{
		if (condition)
			throw new NotSupportedException(
				"The operation is not supported by current Revit API version. Failed to retrieve the application context.");
	}
}